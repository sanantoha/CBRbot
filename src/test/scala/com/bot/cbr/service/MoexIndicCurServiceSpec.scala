...
class MoexIndicCurServiceSpec extends UnitSpec {
  ...
  val expIndicCur = Vector(
    MoexCurrency("EUR/RUB", LocalDateTime.of(2019, 1, 11, 18, 30, 0), BigDecimal(76.9564)).rightNec[CBRError],
    MoexCurrency("EUR/RUB", LocalDateTime.of(2019, 1, 11, 13, 45, 0), BigDecimal(77.1318)).rightNec[CBRError],
    MoexCurrency("EUR/RUB", LocalDateTime.of(2019, 1, 10, 18, 30, 0), BigDecimal(77.3575)).rightNec[CBRError],
    MoexCurrency("EUR/RUB", LocalDateTime.of(2019, 1, 10, 13, 45, 0), BigDecimal(77.2179)).rightNec[CBRError]
  )
  ...
  def runMoexIndicCurService[F[_]: ConcurrentEffect](response: String): F[Vector[EitherNec[CBRError, MoexCurrency]]] = {
    val metals = for {
      client <- Stream.emit(mkClient[F](response)).covary[F]
      logger <- Stream.emit(NoOpLogger.impl[F]).covary[F]
      config = Config("url", "url", "url", "url")
  ...
  def runTest[F[_]: ConcurrentEffect: ContextShift](): F[Vector[EitherNec[CBRError, MoexCurrency]]] = {
